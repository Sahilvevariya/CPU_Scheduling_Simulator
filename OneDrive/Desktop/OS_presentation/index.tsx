...
class CPUSimulator {
    ...
    private bindDOMEvents() {
        ...
        // Event listener for all process table inputs. 'change' fires when focus is lost.
        document.getElementById('simulator-section').addEventListener('change', (e) => this.handleProcessInputChange(e));
    ...
    private handleProcessInputChange(e: Event) {
        const target = e.target as HTMLInputElement;
        if (!target.matches('input[data-pid]')) return;

        const pid = parseInt(target.dataset.pid, 10);
        const field = target.dataset.field as keyof CPUProcess;
        const value = parseInt(target.value, 10);

        const process = this.processes.find(p => p.id === pid);
        if (process && !isNaN(value)) {
            if ((field === 'arrivalTime' && value >= 0) || ((field === 'burstTime' || field === 'priority') && value > 0)) {
                (process[field] as number) = value;
            }
        }
        // If the user switches tabs, renderActiveSection will re-render the table with the new data.
    }
    ...
    private initializeSimulatorView() {
        ...
        this.renderProcessTable('#fcfs-process-table', true);
        this.renderProcessTable('#sjf-process-table', true);
        this.renderProcessTable('#priority-process-table', true);
    ...
    private runSimulation(algorithm: 'fcfs' | 'sjf' | 'priority') {
        // Data is now always up-to-date in this.processes, no need to read from table here.
    ...
    private executeSchedule(processQueue: CPUProcess[]): { scheduledProcesses: CPUProcess[], metrics: Metrics } {
        ...
        // Create a copy to avoid mutating the sorted array properties directly
        const localProcessQueue = JSON.parse(JSON.stringify(processQueue));

        for (const process of localProcessQueue) {
        ...
        const metrics: Metrics = {
            avgWaitingTime: totalWaitingTime / localProcessQueue.length,
            avgTurnaroundTime: totalTurnaroundTime / localProcessQueue.length
        };
    ...
    private renderActiveSection() {
        ...
        // Re-render table if it's an editable one, to sync data from this.processes across tabs
        if (['fcfs', 'sjf', 'priority'].includes(this.activeSection)) {
            this.renderProcessTable(`#${this.activeSection}-process-table`, true);
        }
    ...
    private renderProcessTable(containerId: string, isEditable: boolean) {
    ...
    private renderGanttChart(data: CPUProcess[], containerId: string) {
        ...
        rects.transition()
            .duration(800)
            .delay((d, i) => i * 100) // Animate in sequence
        ...
        labels.transition()
            .duration(500)
            .delay((d, i) => i * 100 + 400) // Fade in after the bar starts drawing
            .attr('opacity', 1);
    }
